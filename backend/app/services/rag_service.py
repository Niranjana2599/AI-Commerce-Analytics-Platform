"""Traceable RAG orchestration with LangChain FAISS and local Ollama."""
...
from backend.app.llmops.langsmith_tracing import (
    RAG_TAGS,
    trace_metadata,
    traceable,
)
from backend.app.llmops.observability import (
    build_event,
    log_rag_event,
)
...
FAISS_DIR = Path(settings.models_dir) / "faiss_ecommerce"
    ...
    if _vectorstore is None:
        if not (FAISS_DIR / "index.faiss").exists():
            raise FileNotFoundError(
                f"FAISS index not found: {FAISS_DIR}"
            )
...
def _retrieve(
    ...
    if not results:
        raise ValueError(
            "FAISS returned no relevant documents."
        )
...
@traceable(
    ...
    run_type="prompt",
    tags=RAG_TAGS + ["prompt"],
)
def _render_prompt(
    question: str,
    context: str,
) -> tuple[str, str]:
    """Render the reusable versioned RAG prompt."""

    prompt, definition = build_rag_prompt(
        question,
        context,
    )
...
@traceable(
    name="rag_output_parser",
    run_type="parser",
    tags=RAG_TAGS + ["parser"],
)
def _parse_answer(
    answer: str,
) -> str:
...
# ============================================================
# FAISS knowledge-base version
# ============================================================

def _faiss_version() -> str:
    """Return a lightweight FAISS index version."""
    ...
    return (
        f"faiss-{index_file.stat().st_mtime_ns}"
    )
...
def run_rag_pipeline(
    ...
    context, sources, similarity_scores = _retrieve(
        question,
        limit,
    )
    ...
    prompt, prompt_version = _render_prompt(
        question,
        context,
    )
...
def answer_question(
    ...
    """Return the public RAG answer and record observability."""
